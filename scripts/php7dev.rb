class Php7dev
  def Php7dev.configure(config, settings)
    ...
    config.vm.network "forwarded_port", guest: 80, host: settings["http_port"] ||= 8000
    config.vm.network "forwarded_port", guest: 443, host: settings["https_port"] ||= 44300
    config.vm.network "forwarded_port", guest: 3306, host: settings["mysql_port"] ||= 33060
    ...
    if settings['folders'].kind_of?(Array)
      settings["folders"].each do |folder|
        if folder.has_key?("owner") and folder.has_key?("group")
            config.vm.synced_folder folder["map"], folder["to"], owner: folder["owner"], group: folder["group"], type: folder["type"] ||= nil
        else
    ...
    # Register All of the Configured Shared Files
    if settings['files'].kind_of?(Array)
      settings["files"].each do |file|
        if File.exists? file["source"] and file.has_key?("destination") then
